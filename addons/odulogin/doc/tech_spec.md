...
`odulogin` is an independent Odoo 18 implementation of the temporary user
...
The OWL component `odulogin.OduLoginSystray` is registered in the Odoo 18
...
Use branch `18.0` for a fresh installation on Odoo 18. Install the module from `addons` together with its declared dependencies. This branch does not downgrade an existing Odoo database. Window actions use `groups_id`; HTTP tests use JSON request bodies and host-scoped cookies to follow session rotation.
