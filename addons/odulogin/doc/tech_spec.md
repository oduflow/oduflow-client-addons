...
`odulogin` is an independent Odoo 17 implementation of the temporary user
...
The OWL component `odulogin.OduLoginSystray` is registered in the Odoo 17
...
Use branch `17.0` for a fresh installation on Odoo 17. Install the module from `addons` together with its declared dependencies. This branch does not downgrade an existing Odoo database. Window actions use `groups_id`; HTTP tests use JSON request bodies and host-scoped cookies to follow session rotation.
