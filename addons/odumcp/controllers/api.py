...
class OduMcpController(http.Controller):
    @http.route(
        "/odumcp/v1/health",
        type="http",
        auth="public",
    ...
    def capabilities(self):
        ...
        if response:
            return response
        body, status = request.env["odumcp.service"].execute_request(
            access,
            "capabilities",
            {},
            request_id,
    ...
    def event_ticket(self):
        ...
        if response:
            return response
        token = request.env["odumcp.event.ticket"].sudo()._issue(access)
        return self._response(
            {
                "ok": True,
                "request_id": request_id,
                "data": {
                    "ticket": token,
                    "expires_in": request.env["odumcp.event.ticket"]._ttl_seconds(),
                },
            },
            201,
        )
    ...
    def _access(self, request_id):
        access, error = request.env["res.users"]._mcp_for_user(request.env.user)
        if error:
