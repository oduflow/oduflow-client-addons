...
class OduMcpApproval(models.Model):
    ...
    _request_uid_unique = models.Constraint(
        "UNIQUE(request_uid)", "The approval request identifier must be unique."
    )
    _idempotency_unique = models.Constraint(
        "UNIQUE(user_id, idempotency_key)",
        "The idempotency key has already been used by this MCP user.",
    )
    ...
    def _public_dict(self):
        ...
        return {
            ...
            "approval_url": (
                f"{self.get_base_url().rstrip('/')}/odoo/"
                f"action-{self.env.ref('odumcp.action_odumcp_approvals').id}/{self.id}"
            ),
