...
class IrHttp(models.AbstractModel):
    ...
    def _auth_method_mcp_event(cls):
        ...
        match = re.fullmatch(r"Bearer\s+(.+)", header, re.IGNORECASE)
        ticket = (
            request.env["odumcp.event.ticket"].sudo()._check(match.group(1))
            if match
            else request.env["odumcp.event.ticket"].browse()
        )
        if not ticket:
            raise Unauthorized(
        ...
        context["odumcp_event_channel"] = ticket.user_id.mcp_event_channel
