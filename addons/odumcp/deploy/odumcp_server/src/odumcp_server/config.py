...
def _env(name: str, default: str = "") -> str:
    return os.getenv(f"ODUMCP_{name}", default).strip()
...
def _int(name: str, default: int, *, minimum: int, maximum: int) -> int:
    ...
    try:
        ...
        raise ConfigurationError(f"ODUMCP_{name} must be an integer.") from exc
    if not minimum <= parsed <= maximum:
        raise ConfigurationError(f"ODUMCP_{name} must be between {minimum} and {maximum}.")
    return parsed
...
def _float(name: str, default: float, *, minimum: float, maximum: float) -> float:
    ...
    try:
        ...
        raise ConfigurationError(f"ODUMCP_{name} must be numeric.") from exc
    if not minimum <= parsed <= maximum:
        raise ConfigurationError(f"ODUMCP_{name} must be between {minimum} and {maximum}.")
    return parsed
...
class Settings:
    ...
    def validate(self) -> None:
        ...
        if parsed.scheme not in {"http", "https"} or not parsed.netloc:
            raise ConfigurationError("ODUMCP_ODOO_URL must be an absolute HTTP(S) URL.")
        if self.events_url:
            events = urlparse(self.events_url)
            if events.scheme not in {"http", "https"} or not events.netloc:
                raise ConfigurationError("ODUMCP_EVENTS_URL must be an absolute HTTP(S) URL.")
        if not self.mcp_path.startswith("/"):
            raise ConfigurationError("ODUMCP_MCP_PATH must start with '/'.")
        if self.queue_scope not in {"global", "user"}:
            raise ConfigurationError("ODUMCP_QUEUE_SCOPE must be 'global' or 'user'.")
