...
async def _execute(
    ...
    if access_token is None or not access_token.subject:
        raise ToolError("authentication_required: A valid OduMCP API key is required.")
    try:
...
def _change_params(
    ...
    """Собрать params плана: batch_key едет рядом с планом, а не внутри payload."""
    params: dict[str, Any] = {
        "action": action,
        "payload": payload,
        "idempotency_key": values["idempotency_key"],
    }
    if values.get("batch_key"):
        params["batch_key"] = values["batch_key"]
    return params
...
def _register_health_routes(
    ...
    *,
    ...
    async def healthz(_request: Request) -> JSONResponse:
        ...
        return JSONResponse({"status": "ok", "service": "odumcp-server"})
...
def _register_core(mcp: FastMCP) -> None:
    ...
    async def odoo_search_records(
        ...
        Take field names from odoo_describe_model: one unknown or policy-denied
        name rejects the whole call and names the offenders in the error. Under a
        profile with partial field reads the call succeeds instead and reports the
        skipped names in omitted_fields — treat those fields as unknown, not empty.
        """
        params = _validated(
            SearchRequest,
            {
    ...
    async def odoo_get_record(
        ...
        Take field names from odoo_describe_model. One unknown or policy-denied
        name, or one unavailable ID, rejects the whole call; the error lists the
        offending names or IDs, so retry without them instead of guessing. Under a
        profile with partial field reads a field list succeeds instead and reports
        the skipped names in omitted_fields — treat those fields as unknown, not
        empty.
        """
        params = _validated(
            ReadRequest,
    ...
    async def odoo_aggregate_records(
        ...
        Take field and groupby names from odoo_describe_model: one unknown or
        policy-denied name rejects the whole call.
        """
        params = _validated(
            AggregateRequest,
            {
...
def _register_write(mcp: FastMCP) -> None:
    ...
    async def odoo_preview_create(
        ...
        Pass the same batch_key for every plan of one user request so Odoo groups them into a single request that can be approved in one go.
        """
        ...
        return await _execute(
            ...
            _change_params(
                "record.create",
                {"model": values_dict["model"], "values": values_dict["values"]},
                values_dict,
            ),
        )
    ...
    async def odoo_preview_delete(
        ...
        return await _execute(
            ...
            _change_params(
                "record.delete",
                {"model": values_dict["model"], "ids": values_dict["ids"]},
                values_dict,
            ),
        )
    ...
    async def odoo_preview_method(
        ...
        payload.pop("batch_key", None)
        return await _execute(
            ctx,
            "changes.preview",
            _change_params("method.call", payload, values_dict),
        )
...
def _register_collaboration(mcp: FastMCP) -> None:
    ...
    async def odoo_preview_post_message(
        ...
        return await _execute(
            ctx,
            "changes.preview",
            _change_params("message.post", payload, values),
        )
    ...
    async def odoo_preview_schedule_activity(
        ...
        return await _execute(
            ctx,
            "changes.preview",
            _change_params("activity.schedule", payload, values),
        )
...
def _register_documents(mcp: FastMCP) -> None:
    ...
    async def odoo_preview_upload_attachment(
        ...
        return await _execute(
            ctx,
            "changes.preview",
            _change_params("attachment.create", payload, values),
        )
...
def _register_prompts(mcp: FastMCP) -> None:
    ...
    def investigate_access_denial(operation: str, model: str) -> str:
        return (
            ...
            "Read the error code first: unknown_field means the name does not exist in "
            "this Odoo version, record_not_found means the record is gone, and only "
            "field_denied, access_denied and policy_denied are policy decisions. "
            "Check whoami, list_models, and describe_model. Do not attempt alternate models, "
            "fields, methods, or domains to bypass a policy denial. "
