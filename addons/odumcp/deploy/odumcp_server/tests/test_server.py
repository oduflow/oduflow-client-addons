...
import odumcp_server.server as server_module
from odumcp_server.config import Settings
from odumcp_server.server import create_server
from odumcp_server.subscriptions import OdooEventBridge
...
TOOL_CASES = [
    ...
    (
        "odoo_preview_update",
        {
            ...
            "ids": [1],
            "values": {"name": "Updated"},
            "idempotency_key": "update-02",
            "batch_key": "cleanup-2026-09",
        },
        "changes.preview",
        {
            "action": "record.update",
...
async def test_http_health_and_readiness_routes() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/odumcp/v1/health"
        assert "Authorization" not in request.headers
        return httpx.Response(
            200,
            json={"status": "ok", "service": "odumcp"},
            request=request,
        )
...
@pytest.mark.asyncio
async def test_verified_requests_revive_the_odoo_event_watcher(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ensured: list[str | None] = []

    async def ensure_watcher(_bridge: OdooEventBridge, access_token) -> None:
        ensured.append(access_token.subject)

    monkeypatch.setattr(OdooEventBridge, "ensure_watcher", ensure_watcher)

    def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/odumcp/v1/identity"
        return httpx.Response(
            200,
            json={
                "ok": True,
    ...
    server = create_server(
        _settings(events_enabled=True),
        auth_transport=httpx.MockTransport(handler),
    )
    try:
        first = await server.auth.verify_token("user-key")
        cached = await server.auth.verify_token("user-key")
    finally:
        await server.auth.aclose()

    assert first is not None
    assert cached is not None
    assert ensured == ["odoo:prod:42", "odoo:prod:42"]
