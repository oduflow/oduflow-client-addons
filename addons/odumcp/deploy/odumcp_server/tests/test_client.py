...
from odumcp_server.client import CircuitBreaker, CircuitPermit, OdooControlClient
from odumcp_server.config import Settings
from odumcp_server.errors import CircuitOpenError, OdooApiError, ResponseTooLargeError
...
async def test_safe_operation_retries_but_mutation_does_not(
    ...
    monkeypatch.setattr("odumcp_server.client.asyncio.sleep", no_sleep)
...
async def test_user_errors_and_large_responses_do_not_open_breaker() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        assert request.headers["Content-Type"] == "application/vnd.odumcp+json"
...
@pytest.mark.asyncio
async def test_error_details_reach_the_client_and_the_tool_error_text() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            422,
            json={
                ...
                "error": {
                    "code": "unknown_field",
                    "message": "Model res.partner has no field nope.",
                    "data": {"unknown_fields": ["nope"], "denied_fields": []},
                },
            },
            request=request,
        )
    ...
    try:
        with pytest.raises(OdooApiError) as raised:
            await client.execute("records.read", bearer_token="key")
    finally:
        await client.aclose()

    error = raised.value
    assert error.code == "unknown_field"
    assert error.data == {"unknown_fields": ["nope"], "denied_fields": []}
    assert "nope" in str(error)
    assert error.as_dict()["error"]["data"]["unknown_fields"] == ["nope"]
...
async def test_client_context_manager_and_capabilities_unwrap() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/odumcp/v1/capabilities"
        return httpx.Response(
            200,
