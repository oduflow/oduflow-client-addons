...
from odumcp_server.subscriptions import EventTicket, OdooEventBridge, SubscriptionPublisher
...
async def test_event_bridge_uses_explicit_longpoll_endpoint() -> None:
    bridge = OdooEventBridge(
        _settings(events_url="http://odoo-evented:8072/odumcp/v1/events"),
        SubscriptionPublisher(),
    )

    try:
        assert bridge._events_url() == "http://odoo-evented:8072/odumcp/v1/events"
    finally:
        await bridge.aclose()
...
async def test_event_message_parser_ignores_malformed_or_untrusted_notifications() -> None:
    ...
    invalid_notifications = [
        ...
        {"message": "invalid"},
        {"message": {"type": "other"}},
        {"message": {"type": "odumcp_resource_updated", "payload": "invalid"}},
        {
            "message": {
                "type": "odumcp_resource_updated",
...
async def test_event_bridge_derives_longpoll_url() -> None:
    ...
    try:
        assert insecure._events_url() == "https://odoo.example.test/odumcp/v1/events"
        assert plain._events_url() == "http://odoo.example.test/odumcp/v1/events"
    finally:
