...
def split_title(text):
    """Отделить заголовок первого уровня от остального Markdown.

    Возвращает ``(title, rest)``: ``title`` -- текст заголовка без решёток и
    ``""``, когда документ начинается не с него; ``rest`` -- текст без этой
    строки. Дальше заголовок первого уровня не ищется: у него другая роль --
    он часть текста, а не название документа.
    """
    if not text:
        return "", text or ""
    lines = text.replace("\r\n", "\n").replace("\r", "\n").split("\n")
...
def split_section(text, slug):
    """Вырезать из Markdown раздел с якорем ``slug``.

    Раздел -- это заголовок с таким якорем и всё, что стоит под ним до
    следующего заголовка того же или более высокого уровня; вложенные
    подразделы остаются внутри. Заголовки внутри блоков кода заголовками не
    считаются.

    Возвращает ``(title, body)`` -- текст заголовка и Markdown под ним --
    или ``(None, "")``, когда такого якоря в документе нет.
    """
    ...
    fence_closing = None
    for line in lines:
        if fence_closing is not None:
            if fence_closing.match(line):
                fence_closing = None
        ...
        if marker:
            fence_closing = re.compile(
                r"^\s*%s{3,}\s*$" % re.escape(marker.group(1)[0])
            )
            if title is not None:
                body.append(line)
            continue
        ...
        if heading:
            ...
            if title is None:
                if slugify(heading.group(2)) == slug:
                    title, level = heading.group(2).strip(), found
                continue
            if found <= level:
                break
        if title is not None:
...
def md_to_html(text):
    ...
    while i < n:
        ...
        if heading:
            ...
            raw = heading.group(2)
            out.append('<h%d id="%s">%s</h%d>' % (level, slugify(raw), _inline(raw), level))
            i += 1
            continue
...
def _inline(text):
    ...
    # URL уже экранирован, но схема всё равно проверяется: HTML вставляется
    # клиентом через t-raw и не должен исполнять javascript:/data:.
    text = re.sub(
        r"!\[([^\]]*)\]\(([^)\s]+)\)",
    ...
    # technical identifiers like res_partner or odubook.
...
def slugify(text):
    """Якорь заголовка: то же значение стоит в ``id`` отрендеренного HTML."""
    s = re.sub(r"<[^>]+>", "", text)
