...
This is a port for installation on Odoo 16. It does not migrate an existing Odoo 15 database or rename installed database records. Imported change entries describe the source module's history.
...
The bridge polls `/odupilot/bridge/poll` over the ordinary Odoo HTTP endpoint. Only its service account can call it; callers cannot select arbitrary bus channels. Empty polls wait one second in the bridge. The durable XML-RPC command queue remains the source of truth. No Odoo 15 long-polling route is used. Configure the standard Odoo 16 WebSocket endpoint for browser Discuss events.
